...
class StringStabilityTrainer(pl.LightningModule):
    ...
    def vector_lyapunov_conditions(self,states,disturbances):
        ...
        return torch.stack(V_decreases), V_current[:,:iter_num], control_dist
...
def train_model(num_uavs=3, controlled_indices=None, state_dims=None, control_dims=None, 
    ...
    controller = CombinedController(input_dim=2*system.dim, output_dim=system.dim, shared_Lyapunov=shared_Lyapunov, num_UAVs=num_uavs)
    ...
    V_net = VectorLyapunovNetwork(input_dim=error_dim, hidden_dim=64, G=G, num_UAVs=num_uavs, shared_Lyapunov=shared_Lyapunov)

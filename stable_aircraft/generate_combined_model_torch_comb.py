...
class CombinedUAVNetwork(nn.Module):
    ...
    def _build_selection_matrices(self):
        ...
        Support multiple UAVs in a string formation (3 UAVs).
        """
        dim = self.dim
        ...
        self.uav_selection_matrices_1 = self.uav_selection_matrices[0]
        self.uav_selection_matrices_2 = self.uav_selection_matrices[1]
        self.uav_selection_matrices_3 = self.uav_selection_matrices[2]
        ...
        # Matrix to select control for first follower
        control_selector_1 = torch.zeros(2*self.dim, self.dim)
        control_selector_1[:self.dim, :] = torch.eye(self.dim)
        self.control_selection_matrices_1 = control_selector_1
        
        # Matrix to select control for second follower
        control_selector_2 = torch.zeros(2*self.dim, self.dim)
        control_selector_2[self.dim:, :] = torch.eye(self.dim)
        self.control_selection_matrices_2 = control_selector_2
    ...
    def forward(self, x):
        ...
        x_current_1 = x@self.uav_selection_matrices_1
        x_current_2 = x@self.uav_selection_matrices_2
        x_current_3 = x@self.uav_selection_matrices_3
        # Extract control signals using matrix multiplication
        control_1 = control @ (self.control_selection_matrices_1*0)
        control_2 = control @ self.control_selection_matrices_1  # Control for first follower
        control_3 = control @ self.control_selection_matrices_2  # Control for second follower
        next_state_1 = self.system_dynamics(x_current_1, control_1)
        next_state_2 = self.system_dynamics(x_current_2, control_2)
        next_state_3 = self.system_dynamics(x_current_3, control_3)

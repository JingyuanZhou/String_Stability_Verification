...
class StringStabilityTrainer(pl.LightningModule):
    def __init__(self, V_net, controllers, system, current_index, learning_rate=1e-3, original_controller = None, critics = None, shared_Lyapunov = False, train_additive = False, intitial_platoon_num = 5, additive_platoon_num = 5):
        super().__init__()
    ...
    def vector_lyapunov_conditions(self, states, x_stars, disturbances):
        ...
        if self.train_additive:
            start_index = self.intitial_platoon_num

        else:
            start_index = 1
        check_length = self.intitial_platoon_num + self.additive_platoon_num

        for i in range(start_index, check_length):  # Skip leading vehicle
    ...
    def configure_optimizers(self):
        ...
        if self.current_index > 0 and not self.train_additive:
...
def train_model(num_vehicles, cav_indices, state_dims, control_dims, dynamics_params, learning_rate, batch_size, num_epochs, system_dynamics_network=None, train_system=False, index = 0, pre_trained_model = None, pre_trained_critics = None, shared_Lyapunov = False,
                train_additive = False, load_additive_path = None, intitial_platoon_num = 5, additive_platoon_num = 5):
    """
    ...
    for i in range(len(controllers)):
        ...
        for k, v in parameters.items():
            if k.startswith('controllers'):
                new_key = k.replace('controllers.1.', '')
                load_controller_parameter[new_key] = v

        if i in cav_indices:
            controllers[i].load_state_dict(load_controller_parameter)
    ...
    trainer = StringStabilityTrainer(V_net, controllers, system, 
                                   learning_rate=learning_rate, current_index = index, original_controller = original_controllers, critics = critics, shared_Lyapunov = shared_Lyapunov,train_additive = train_additive
                                   ,intitial_platoon_num = intitial_platoon_num, additive_platoon_num = additive_platoon_num)
...
def retrain_model(num_vehicles, cav_indices, state_dims, control_dims, in_system,
    ...
    trainer = StringStabilityTrainer(V_net, controllers, system, 
                                ...
                                train_additive = train_additive, intitial_platoon_num = intitial_platoon_num, additive_platoon_num = additive_platoon_num)

...
# System parameters
num_vehicles = intitial_platoon_num + additive_platoon_num
...
while (len(ret) > 0) and (index < max_iters):
    ...
    controllers, system, V_net = retrain_model(num_vehicles=num_vehicles, cav_indices=cav_indices, state_dims=state_dims, 
                                               ...
                                               train_additive = train_additive, load_additive_path = load_additive_path, intitial_platoon_num = intitial_platoon_num, additive_platoon_num = additive_platoon_num)
    end_train_time = datetime.now()
    diff = end_train_time - st_train_time
    print("Total training time for model index", str(index), ":", str(diff.seconds))
    num_training_time_list.append(diff.seconds)
...
if train_additive:
    np.save("data/num_ce_list_additive_"+str(num_vehicles)+".npy", num_ce_list)
    np.save("data/num_veri_time_list_additive_"+str(num_vehicles)+".npy", num_veri_time_list)
    np.save("data/num_training_time_list_additive_"+str(num_vehicles)+".npy", num_training_time_list)
    np.save("data/total_time_additive_"+str(num_vehicles)+".npy", total_time)
else:

...
class CombinedNetwork(nn.Module):
    def __init__(self, controllers, V_net, cav_indices, state_dims, system_dynamics):
        ...
        # 状态选择矩阵 - Dynamic approach for any number of HDVs
        self.W_state_with_preceding = []
        
        for i, hdv_idx in enumerate(self.hdv_indices):
            # Create selection matrix for each HDV
            W = torch.zeros(2 * self.num_vehicles, 3)
            
            # Set elements for the HDV
            W[hdv_idx * 2, 0] = 1.0         # Position/spacing of HDV
            W[hdv_idx * 2 + 1, 1] = 1.0     # Velocity of HDV
            
            # Set element for the preceding vehicle's velocity
            W[hdv_idx * 2 + 1, 2] = 1.0     # Velocity of preceding vehicle
            
            # Register buffer with unique name
            self.register_buffer(f'W_state_with_preceding_{i+1}', W)
            self.W_state_with_preceding.append(getattr(self, f'W_state_with_preceding_{i+1}'))
    ...
    def forward(self, x):
        ...
        # 获取CAV控制输出
        x_control = x[:,0:5,:]
        x_control_star = x_stars[:,0:5,:]
        controllers_output = self.CombindedControllers(x_control, x_control_star, u_star, u_bounds)
        
        # 获取HDV加速度
        x_with_preceding = torch.cat([torch.matmul(x.view(batch_size, -1), W) for W in self.W_state_with_preceding], dim=1)
        hdv_acc = torch.cat([self.system_dynamics(x_with_preceding[:,i*3:(i+1)*3]) for i in range(len(self.hdv_indices))], dim=1)
...
def combined_model(V_net, controllers, system_dynamics, output_file, state_dims, cav_indices): 
    ...
    #dummy_input_x = torch.tensor([[20,15]*num_vehicles],requires_grad=True, dtype=torch.float32)  # [1, num_vehicles]

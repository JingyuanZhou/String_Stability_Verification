...
class CombinedNetwork(nn.Module):
    def __init__(self, controllers, V_net, cav_indices, state_dims, system_dynamics,device):
        ...
        # 状态选择矩阵
        W_state_with_preceding_1 = torch.zeros(2 * self.num_vehicles, 3).to(device)
        W_state_with_preceding_1[self.hdv_indices[0] * 2 + 1, 2] = 1.0
        W_state_with_preceding_1[self.hdv_indices[0] * 2, 0] = 1.0
        W_state_with_preceding_1[self.hdv_indices[0] * 2 + 1, 1] = 1.0
        W_state_with_preceding_2 = torch.zeros(2 * self.num_vehicles, 3).to(device)
        W_state_with_preceding_2[self.hdv_indices[1] * 2 + 1, 2] = 1.0
        W_state_with_preceding_2[self.hdv_indices[1] * 2, 0] = 1.0
        W_state_with_preceding_2[self.hdv_indices[1] * 2 + 1, 1] = 1.0
        W_state_with_preceding_3 = torch.zeros(2 * self.num_vehicles, 3).to(device)
        W_state_with_preceding_3[self.hdv_indices[2] * 2 + 1, 2] = 1.0
        W_state_with_preceding_3[self.hdv_indices[2] * 2, 0] = 1.0
        W_state_with_preceding_3[self.hdv_indices[2] * 2 + 1, 1] = 1.0
        ...
        self.register_buffer('W_state_with_preceding_3', W_state_with_preceding_3)

        # CAV选择矩阵
        W_cav = torch.zeros(self.num_vehicles, len(cav_indices)).to(device)
        for i, cav_idx in enumerate(cav_indices):
            W_cav[cav_idx, i] = 1.0
        self.register_buffer('W_cav', W_cav)

        # HDV选择矩阵
        W_hdv = torch.zeros(self.num_vehicles, len(self.hdv_indices)).to(device)
        ...
        self.dt = 0.1
        # 状态转移矩阵A
        self.A = torch.zeros(2 * self.num_vehicles, 2 * self.num_vehicles, dtype=torch.float32).to(device)
        ...
        # 控制输入矩阵B
        self.b = torch.zeros(2 * self.num_vehicles, self.num_vehicles, dtype=torch.float32).to(device)
    ...
    def forward(self, x):
        ...
        x_with_preceding_3 = torch.matmul(x.view(batch_size, -1), self.W_state_with_preceding_3)
        hdv_acc_3 = self.system_dynamics(x_with_preceding_3)
        hdv_acc = torch.cat((hdv_acc_1, hdv_acc_2, hdv_acc_3), dim=1)

        # print(x_with_preceding_1, x_with_preceding_2, x_with_preceding_3)
        ...
        hdv_acc = torch.matmul(self.W_hdv, hdv_acc.transpose(0,1)).transpose(0,1)
        
        # 所有加速度相加
        acceleration = zero_acc + cav_acc + hdv_acc
...
def combined_model(V_net, controllers, system_dynamics, output_file, state_dims, cav_indices, device): 
    """
    ...
    combined_network = CombinedNetwork(controllers, V_net, cav_indices, state_dims, system_dynamics,device).to(device)
    ...
    dummy_input_x = torch.tensor([[[20,15],[21,14],[21,13],[21,13],[21,13]]],requires_grad=True, dtype=torch.float32).to(device)  # [1, num_vehicles]

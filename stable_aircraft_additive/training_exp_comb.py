...
class StringStabilityTrainer(pl.LightningModule):
    def __init__(self, controller, system, V_net, learning_rate=1e-3, 
                 current_index=None, original_controller=None, shared_Lyapunov=True, num_uavs=3,
                 train_additive=False, load_additive_path=None, intitial_platoon_num=3, additive_platoon_num=3):
        super().__init__()
        ...
        # number of controllers are the same as the number of UAVs
        self.num_UAVs = num_uavs
        self.train_additive = train_additive
        self.load_additive_path = load_additive_path
        self.intitial_platoon_num = intitial_platoon_num
        self.additive_platoon_num = additive_platoon_num
    ...
    def configure_optimizers(self):
        ...
        parameters = []
        if not self.train_additive:
            parameters.extend(self.V_net.parameters())
        else:
            for i in range(self.intitial_platoon_num, self.intitial_platoon_num + self.additive_platoon_num):
                parameters.extend(self.V_net.networks[i-1].parameters())
        ...
        if self.current_index > 0:
            if  not self.train_additive:
                ...
                for i in range(self.intitial_platoon_num, self.intitial_platoon_num + self.additive_platoon_num-1):
                    parameters.extend(self.controller.controllers[i].parameters())
    ...
    def vector_lyapunov_conditions(self,states,disturbances):
        ...
        UAV_index = list(range(1,self.num_UAVs))
        ...
        if self.train_additive:
            iter_start = self.intitial_platoon_num
            iter_end = self.intitial_platoon_num + self.additive_platoon_num
        else:
            iter_start = 1
            iter_end = self.intitial_platoon_num + self.additive_platoon_num
        ...
        control_dist += torch.norm(controls - self.original_controller(error_state), dim=1).mean()/100
        
        return torch.stack(V_decreases), V_current[:,iter_start-1:iter_end], control_dist
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_decrease = 2000 * torch.relu(V_decreases + 10*epsilon).mean()
...
def train_model(num_uavs=3, controlled_indices=None, state_dims=None, control_dims=None, 
               dynamics_params=None, learning_rate=1e-3, batch_size=32, num_epochs=100, shared_Lyapunov=True,
               train_additive=False, load_additive_path=None, intitial_platoon_num=3, additive_platoon_num=3):
    """
    ...
    check_point = torch.load(load_additive_path) #229
    parameters = check_point['state_dict']

    
    # For each UAV, either load a pre-trained controller or use None
    controller = CombinedController(input_dim=2*system.dim, output_dim=system.dim, shared_Lyapunov=shared_Lyapunov, num_UAVs=num_uavs, train_additive=train_additive, intitial_platoon_num=intitial_platoon_num, additive_platoon_num=additive_platoon_num)

    # Create a copy for the original controller
    original_controller = CombinedController(input_dim=2*system.dim, output_dim=system.dim, shared_Lyapunov=shared_Lyapunov, num_UAVs=num_uavs, train_additive=train_additive, intitial_platoon_num=intitial_platoon_num, additive_platoon_num=additive_platoon_num)

    if shared_Lyapunov:
        controller.controllers[0].load_state_dict(torch.load("pre_train_model/controller_model.pth"))
        original_controller.controllers[0].load_state_dict(torch.load("pre_train_model/controller_model.pth"))
    else:
        for i in range(num_uavs - 1):
            controller.controllers[i].load_state_dict(torch.load("pre_train_model/controller_model.pth"))
            original_controller.controllers[i].load_state_dict(torch.load("pre_train_model/controller_model.pth"))
    ...
    V_net = VectorLyapunovNetwork(input_dim=error_dim, hidden_dim=64, G=G, num_UAVs=num_uavs, shared_Lyapunov=shared_Lyapunov, train_additive=train_additive, intitial_platoon_num=intitial_platoon_num, additive_platoon_num=additive_platoon_num)
    
    load_controller_parameter = {}
    for i in range(intitial_platoon_num):
        for k, v in parameters.items():
            if k.startswith(f'controller.controllers.{i}.'):
                new_key = k.replace(f'controller.controllers.{i}.', '')
                load_controller_parameter[new_key] = v

    for i in range(intitial_platoon_num):
        controller.controllers[i].load_state_dict(load_controller_parameter)

    for i in range(intitial_platoon_num-1):
        V_parameters = {}
        for k, v in parameters.items():
            if k.startswith(f'V_net.networks.{i}.'):
                new_key = k.replace(f'V_net.networks.{i}.', '')
                V_parameters[new_key] = v
        V_net.networks[i].load_state_dict(V_parameters)
    ...
    trainer = StringStabilityTrainer(
        ...
        num_uavs=num_uavs,
        train_additive=train_additive,
        load_additive_path=load_additive_path,
        intitial_platoon_num=intitial_platoon_num,
        additive_platoon_num=additive_platoon_num
    )
...
class UAVDataModuleRetrain(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        number_of_UAVs = self.counterexamples.shape[1]
        new_data_disturbances = torch.zeros((self.counterexamples.shape[0],number_of_UAVs,3))
...
def retrain_model(in_system, counterexamples, counterexample_ranges, epoch,
                 ...
                 learning_rate=1e-4, batch_size=32, index = 0, shared_Lyapunov=True, num_uavs=3,
                 train_additive=False, load_additive_path=None, intitial_platoon_num=3, additive_platoon_num=3):
    """
    ...
    # Create a copy for the original controller
    original_controller = CombinedController(input_dim=2*system.dim, output_dim=system.dim, shared_Lyapunov=shared_Lyapunov, num_UAVs=num_uavs)
    if shared_Lyapunov:
        original_controller.controllers[0].load_state_dict(torch.load("pre_train_model/controller_model.pth"))
    else:
        for i in range(num_uavs - 1):

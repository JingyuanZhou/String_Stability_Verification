...
train_additive = True
load_additive_path = "model_weights/base_model.ckpt"
intitial_platoon_num = 3
additive_platoon_num = 1

# System parameters
num_UAVs = intitial_platoon_num + additive_platoon_num
...
while (len(ret) > 0) and (index < max_iters):
    ...
    st_train_time = datetime.now()
    controllers, system, V_net = retrain_model(system, ret, ret_ranges, num_epochs, V_net, controller, learning_rate, batch_size, index, shared_Lyapunov=shared_Lyapunov, num_uavs=combined_num_UAVs, train_additive=train_additive, load_additive_path=load_additive_path, intitial_platoon_num=intitial_platoon_num, additive_platoon_num=additive_platoon_num)
    end_train_time = datetime.now()
    diff = end_train_time - st_train_time
    num_training_time_list.append(diff.seconds)
    print("Total training time for model index", str(index), ":", str(diff.seconds))
...
if train_additive:
    np.save("data/num_ce_list_"+ str(num_UAVs) + "_additive.npy", num_ce_list)
    np.save("data/num_veri_time_list_"+ str(num_UAVs) + "_additive.npy", num_veri_time_list)
    np.save("data/num_training_time_list_"+ str(num_UAVs) + "_additive.npy", num_training_time_list)
    np.save("data/num_total_time_list_"+ str(num_UAVs) + "_additive.npy", num_total_time_list)
else:

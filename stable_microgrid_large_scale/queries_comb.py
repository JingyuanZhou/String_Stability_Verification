...
class DecentralizedVerificationQuery:
    def __init__(self, PATH, system, agent_id, num_inverters=3, shared_lyapunov=False):
        """
...
def decentralized_verification(
    ...
    if shared_lyapunov:
        ...
        check_inverters = num_inverters
    for inv_id in range(check_inverters):
        if not shared_lyapunov and inv_id == check_inverters-2:
            continue
        print(f"\n=== Verifying Follower Inverter {inv_id} ===")
        query = DecentralizedVerificationQuery(PATH_TO_ONNX, system, inv_id, num_inverters, shared_lyapunov)
        ...
        split_nums = [3 for _ in range(num_inverters)]  # Different resolutions to try
        split_nums[0] = 4
        ...
        for k_ in range(len(split_nums)):
            delta_range.append(np.linspace(0, np.pi/4, split_nums[k_]))  # Phase angle error range (rad) np.pi/4

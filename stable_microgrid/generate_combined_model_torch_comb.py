...
class CombinedMicrogridNetwork(nn.Module):
    def __init__(self, controller, V_net, controlled_indices, state_dims, system_dynamics):
        ...
        self.x_equilibrium[:, [1, 4, 7]] = 2 * torch.pi * 50
...
def combined_model(V_net, controllers, system_dynamics, output_file, state_dims, controlled_indices): 
    ...
    """
    system_dynamics.dynamics_1.load_state_dict(torch.load(f"pre_train_model/dynamics_model_0.pth"))
    system_dynamics.dynamics_2.load_state_dict(torch.load(f"pre_train_model/dynamics_model_1.pth"))
    system_dynamics.dynamics_3.load_state_dict(torch.load(f"pre_train_model/dynamics_model_2.pth"))

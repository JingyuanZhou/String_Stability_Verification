...
# 加载参数并分离控制器参数
check_point = torch.load(f'model_weights/best_model-v944.ckpt') #229
    ...
    for t in range(time_steps):
        ...
        for i in range(num_vehicles):
            if i in cav_indices:
                ...
                state_control = states[:,0:5,:]
                state_star_control = torch.tensor([20.0, 15.0]*5)

                control = controllers[i](state_control, state_star_control, u_star, u_bounds)
...
for vehicle_idx in range(2):#num_vehicles-1
    ...
    V_net = VectorLyapunovNetwork(state_dim=state_dims, G=G,shared_Lyapunov = shared_Lyapunov)
    V_parameters = {}
    for k, v in parameters.items():
    ...
    for i, s in enumerate(spacing_space):
        for j, v in enumerate(velocity_space):
            if vehicle_idx == 0:
                x = torch.tensor([[20.0, 15.0, s, v, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0]], dtype=torch.float32)
                x_star = torch.tensor([[20.0, 15.0]*10], dtype=torch.float32)
                V[i, j] = V_net(x, x_star)[0][0].item()
            else:
                x = torch.tensor([[20.0, 15.0, 20.0, 15.0, s, v, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0, 20.0, 15.0]], dtype=torch.float32)
                x_star = torch.tensor([[20.0, 15.0]*10], dtype=torch.float32)
